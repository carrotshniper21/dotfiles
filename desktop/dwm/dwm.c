...
void manage(Window w, XWindowAttributes *wa) {
  ...
  XMoveResizeWindow(dpy,
                    c->win, c->x + 2 * sw, c->y, c->w, c->h); /* some windows
                                                                 require this */
...
void runautostart(void) {
  ...
  /* if $XDG_DATA_HOME is set and not empty, use $XDG_DATA_HOME/dwm, otherwise
     use
  ...
  if (!(stat(pathpfx, &sb) == 0 && S_ISDIR(sb.st_mode))) {
    /* the XDG conformant path does not exist or is no directory so we try
     * ~/.dwm instead */
