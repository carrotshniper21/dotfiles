...
post_install() {
    ...
    cd .. && rm -rf yay || exit
    yay -S neofetch skippy-xd cava ranger mpv rofi wezterm zsh feh xorg-xrandr neovim flameshot notify-osd discord chromium xclip pavucontrol ttf-roboto-mono-nerd thunar lxappearance fzf networkmanager exa bat github-cli zsh-autosuggestions zsh-syntax-highlighting meson ninja uthash libconfig nodejs npm python-pip libsixel imagemagick qt5‑graphicaleffects qt5‑quickcontrols2 qt5‑svg --noconfirm || exit
...
configure_dots() {
    ...
    sudo mv desktop/grub /etc/default/grub || exit
    sudo grub-mkconfig -o /boot/grub/grub.cfg || exit
    sudo cp -r desktop/sugar-candy /usr/share/sddm/themes || exit
    sudo mv desktop/default.conf /usr/lib/sddm/sddm.conf.d/default.conf || exit
